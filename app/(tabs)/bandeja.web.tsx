...
const ANIM = `
  ...
  .b-row:hover { background: rgba(244,80,30,0.06) !important; }
  .b-btn { transition: transform 0.15s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.15s ease, filter 0.15s ease, box-shadow 0.15s ease; cursor: pointer; }
  .b-btn:hover:not(:disabled) { transform: translateY(-1px); filter: brightness(1.04); box-shadow: 0 4px 14px rgba(40,30,24,0.12); }
  .b-btn:active:not(:disabled) { transform: translateY(0) scale(0.97); opacity: 0.9; transition-duration: 0.08s; }
  ...
  return (
    <div className="b-modal-overlay" onClick={() => { if (!enviando) onClose(); }}
      ...
      <div className="b-modal" onClick={(e) => e.stopPropagation()}
        style={{ background: T.panel, border: `1px solid ${T.borderHi}`, borderRadius: isMobile ? '16px 16px 0 0' : 16, padding: isMobile ? '18px 18px 84px' : 24, width: '100%', maxWidth: 560, maxHeight: isMobile ? 'calc(92vh / var(--mecha-zoom, 1))' : '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 70px rgba(40,30,24,0.35)' }}>
