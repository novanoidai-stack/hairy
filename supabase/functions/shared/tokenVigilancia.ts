...
// secreto propio cuyo peor uso posible es ensuciar la pestana Salud.
...
export function autorizarVigilancia(req: Request, quien: string): Veredicto {
  ...
  if (!esperado) {
    ...
    return {
      ok: false,
      status: 500,
      cuerpo: { error: 'sin_configurar', porque: 'falta VIGILANCIA_TOKEN' },
    };
  }
  ...
  if (!igualesEnTiempoConstante(recibido, esperado)) {
    ...
    console.warn(
      `[${quien}] rechazada. x-vigilancia-token:`,
      recibido ? `len=${recibido.length}` : 'AUSENTE',
    );
    return {
      ok: false,
      status: 401,
      cuerpo: { error: 'no_autorizado', porque: 'x-vigilancia-token no coincide' },
    };
  }
