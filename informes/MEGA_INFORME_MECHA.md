...
**S03 HECHA (9 jul):** rediseño del **panel + chat** de Chispa — se ve de un vistazo **quién habla y cuándo** (autor + hora por turno, mensajes agrupados por autor, separación clara entre turnos, avatar solo en el primero del grupo), **identidad** con punto de estado "en línea" + "IA de tu salón", animaciones cuidadas (typing con cabecera, pulso de estado, transición de pantalla completa, todo bajo `prefers-reduced-motion`), y el conflicto **onboarding-vs-chat** resuelto sin ambigüedad (banner siempre visible "Configuración guiada en curso" + Saltar/Salir; el chat se reanuda al salir). Limpieza de emojis y de `BIENVENIDA` muerto. Solo cliente (no toca edge). Verificado E2E en la demo (móvil+escritorio); manuales/`iaCatalogo`/specs al día.
...
## Adenda — Capa IA "Chispa" Sesion 11-A: Migracion Magica + Vision — HECHA (6 jul)

Se ha implementado el motor de Migración Mágica para facilitar el onboarding (importando datos estructurados de imágenes, CSV o PDF vía LLM de visión).
...
- **IA Chispa V3 · S14 (avisos accionables + canal urgente) — HECHA (9 jul):** los hallazgos de S13 se muestran en la campana de Avisos en una seccion "Chispa esta vigilando" (aditiva, sin duplicar las secciones nativas) con acciones ver/resolver/descartar y marca URGENTE, en web (AvisosBell) y movil (AvisosSheet). Outbox `hallazgos_notificaciones` (regla urgente: cita sin confirmar en <12h) + RPCs service_role `notificaciones_hallazgos_pendientes`/`marcar_notificacion_hallazgo_enviada` como stub; el envio real (WhatsApp) es de Alexandro. Verificado E2E por SQL + smoke UI.

- **IA Chispa V3 · S15 (iniciativa / próxima mejor acción) — HECHA (9 jul):** Chispa toma la iniciativa con tacto: un launcher global (ProximaAccionLauncher.web.tsx en _layout) lee la página actual + los hallazgos de S13 y propone de forma discreta la acción más valiosa (selector determinista lib/proximaAccion.ts, sin LLM), con "Hacer" de un clic o "Ahora no" (silencia 24h reutilizando chispa_memoria S09, aprende del descarte). Sin migración nueva. Verificado test selector 6/6 + smoke demo.
