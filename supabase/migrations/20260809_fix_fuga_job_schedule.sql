-- Fix: procesar_alertas_fuga() con umbral frecuencia_dias >= 2 para evitar avisos falsos
-- Issue: fichas de cliente con ~200 citas completadas pero varias el mismo día generaban
-- frecuencia_dias=0 o 1, produciendo (0 * 1.4 = 0) y alertas basura para cualquier cliente.
-- Solución: excluir clientes con ciclo < 2 días (igual que lib/informes/retencionClientes.ts)
...
begin
  ...
  -- FIX (9 ago 2026): Excluir fichas con frecuencia_dias < 2 para evitar avisos
  -- falsos generados por clientes con múltiples citas el mismo día.
  with en_riesgo as (
    select
      cl.id as cliente_id,
      cl.negocio_id,
...
-- Programar job de pg_cron: ejecutar procesar_alertas_fuga() todos los días a las 4:30 AM
-- (ASISTENCIA: no corre si nadie tiene fugaClientasActivo=true, pero el recalculo de
-- agregados sí corre, así que beneficia a toda la red aunque el toggle esté off)
select cron.schedule(
  'mecha_procesar_alertas_fuga',
  '30 4 * * *',
