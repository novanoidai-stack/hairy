...
**Goal:** El portal público de reservas (`disponibilidad_publica`, `portal_dias_disponibles`) deja de bloquear el reposo de una cita existente como si fuera trabajo activo, y anota con el minutaje exacto los huecos que nacen de un reposo.

**Architecture:** Dos funciones SQL (`SECURITY DEFINER`, proyecto Supabase "Mecha") se reescriben para chocar solo contra las fases activas de cada cita (`[inicio, fin_activa)` y, si hay reposo real, `[fin_espera, fin)`), replicando la regla que ya usa `ventanasActivas()` en `lib/retrasos.ts` para la agenda interna. `disponibilidad_publica` gana dos columnas calculadas (`en_reposo`, `reposo_disponible_min`); el cliente TS y la UI del portal las consumen para mostrar una nota neutra.
