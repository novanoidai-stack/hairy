-- RPC de cobro (caja operativa, NO fiscal) usado por la pantalla Caja (app/(tabs)/caja.web.tsx).
-- Aplicada en remoto el 19 jun 2026 como migracion `crear_cobro_desde_cita_rpc`.
-- security definer + comprobacion de negocio (no se puede cobrar citas de otro salon).
-- Descuenta la senal ya pagada (pagos.tipo='senal' pagada) del total, segun el informe.
...
begin
  ...
  insert into public.cobros (
    ...
    coalesce(v_senal_cents, 0), 'pos', 'completado'
...
-- Solo autenticados (no anon/public). Migracion `crear_cobro_desde_cita_revoke_anon`.
revoke execute on function public.crear_cobro_desde_cita(uuid, text, integer, integer) from public;
grant execute on function public.crear_cobro_desde_cita(uuid, text, integer, integer) to authenticated;
