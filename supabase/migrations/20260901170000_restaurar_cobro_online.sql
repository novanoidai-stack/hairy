-- 1 sep 2026. Restaurar el stack de cobro online por QR/enlace (POS-2).
--
-- Copia literal de archive/migraciones-legacy/cobro-online-total-qr.sql: ni
-- la tabla ampliada (cita_pago_enlaces.tipo) ni las 7 funciones vivian en la
-- cadena canonica, igual que paso con servicios_sugeridos. Sin esto, un
-- entorno reconstruido desde supabase/migrations pierde el cobro online
-- (iniciar_cobro_online, pago_info_publica, registrar_cobro_online...).
-- Todo es idempotente: no rompe nada donde ya este aplicado.
