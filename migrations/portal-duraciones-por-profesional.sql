-- Portal publico: respetar los overrides de duracion por profesional
-- ------------------------------------------------------------------
-- Las RPC publicas calculaban la duracion SOLO desde public.servicios, mientras que
-- la app de gestion aplica dos mecanismos de override por profesional:
--   1) public.professional_service_overrides (duracion, duracion_espera_min,
--      duracion_activa_extra_min, activo)  -- prioridad alta
--   2) public.duraciones_profesional (duracion_activa_min, duracion_espera_min,
--      duracion_activa_extra_min)          -- prioridad media
--   3) el catalogo public.servicios                                  -- fallback
-- Consecuencias del fallo (latente: hoy ambas tablas estan vacias):
--   a) si un profesional tarda mas en un servicio, el portal reservaba un bloque
--      demasiado corto y la cita acababa solapando de verdad.
--   b) professional_service_overrides.activo = false significa que ese profesional
--      NO hace ese servicio, y el portal lo seguia ofreciendo.
-- La comparacion de solape NO se toca: sigue siendo la conservadora
-- coalesce(fin_espera, coalesce(fin_activa, fin)).
-- Idempotente.

-- 1) Ayudantes ---------------------------------------------------------------

-- Resuelve la duracion efectiva con la misma precedencia que el cliente.
-- Recibe la base del catalogo ya validada por quien llama (reservable_online,
-- activo, negocio_id) para no duplicar aqui esas reglas.
create or replace function public.duracion_efectiva_profesional(
  p_servicio_id    uuid,
...
returns table (activa int, espera int, extra int, total int)
language sql
stable
security definer
set search_path to 'public'
as $$
  select v.activa, v.espera, v.extra, v.activa + v.espera + v.extra
  from (
    select
      coalesce(pso.duracion, dp.duracion_activa_min, p_base_activa)::int as activa,
      coalesce(pso.duracion_espera_min, dp.duracion_espera_min, p_base_espera, 0)::int as espera,
      coalesce(pso.duracion_activa_extra_min, dp.duracion_activa_extra_min, p_base_extra, 0)::int as extra
    from (select 1) z
    left join public.professional_service_overrides pso
      on pso.service_id = p_servicio_id and pso.professional_id = p_profesional_id
    left join public.duraciones_profesional dp
      on dp.servicio_id = p_servicio_id and dp.profesional_id = p_profesional_id
  ) v;
$$;

-- activo = false en el override significa "este profesional no hace este servicio".
-- Sin fila de override, lo hace (misma regla que serviciosFiltrados en la agenda).
create or replace function public.profesional_ofrece_servicio(
  p_profesional_id uuid,
  p_servicio_id    uuid
)
returns boolean
...
as $$
  select not exists (
    select 1
    from public.professional_service_overrides pso
    where pso.professional_id = p_profesional_id
      and pso.service_id = p_servicio_id
      and pso.activo is false
  );
$$;

-- Solo se llaman DENTRO de RPC security definer: anon no las necesita.
revoke all on function public.duracion_efectiva_profesional(uuid, uuid, int, int, int) from public, anon, authenticated;
revoke all on function public.profesional_ofrece_servicio(uuid, uuid) from public, anon, authenticated;

-- 2) disponibilidad_publica ---------------------------------------------------
-- La duracion deja de ser un escalar: cada profesional puede tener la suya, asi
-- que viaja por las CTEs (profs -> franjas -> gen) como gen.total.
...
-- 3) portal_dias_disponibles --------------------------------------------------
-- Antes sumaba las tres duraciones del catalogo en un solo escalar; ahora las
-- guarda por separado para poder aplicarles el override de cada profesional.
...
begin
  ...
  for v_asistente in select value from jsonb_array_elements(p_asistentes) loop
    ...
    -- Override activo = false: ese profesional no realiza este servicio.
    if not public.profesional_ofrece_servicio(v_prof_id, v_servicio_id) then
      raise exception 'El profesional del asistente % no realiza ese servicio', v_orden;
    end if;

    -- Duracion efectiva: override del profesional por encima del catalogo.
    select d.activa, d.espera, d.extra
      into v_dur, v_espera, v_extra
    from public.duracion_efectiva_profesional(v_servicio_id, v_prof_id, v_dur, v_espera, v_extra) d;
...
-- 6) modificar_cita_publica ---------------------------------------------------

create or replace function public.modificar_cita_publica(
  p_slug text,
  p_cita_id uuid,
  p_telefono text,
  p_nuevo_inicio timestamp with time zone,
  p_nuevo_profesional_id uuid default null::uuid,
  p_canal text default 'web'::text
)
returns jsonb
language plpgsql
...
begin
  ...
  -- Duracion efectiva: override del profesional por encima del catalogo.
  -- Importante al cambiar de profesional: el bloque se recalcula con SU duracion.
  select d.activa, d.espera, d.extra
    into v_dur, v_espera, v_extra
...
-- 7) Grants: las cinco RPC publicas las ejecuta anon (round 4: hay que darlo
--    explicitamente, create or replace no siempre conserva el grant).
grant execute on function public.disponibilidad_publica(text, uuid, date, uuid) to anon, authenticated, service_role;
grant execute on function public.portal_dias_disponibles(text, uuid, uuid, integer) to anon, authenticated, service_role;
grant execute on function public.crear_cita_publica(text, uuid, uuid, timestamptz, text, text, text, text, text, boolean, boolean, text) to anon, authenticated, service_role;
grant execute on function public.crear_cita_publica_grupo(text, timestamptz, text, text, text, jsonb, boolean, text) to anon, authenticated, service_role;
grant execute on function public.modificar_cita_publica(text, uuid, text, timestamptz, uuid, text) to anon, authenticated, service_role;
