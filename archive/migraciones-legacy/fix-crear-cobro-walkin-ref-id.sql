-- Migration: fix-crear-cobro-walkin-ref-id.sql (Mega-Plan WS-9)
--
-- BUG: la venta suelta (walk-in) estaba COMPLETAMENTE ROTA. `v_ref_id` se
-- declara `text` y se insertaba tal cual en `cobro_lineas.ref_id`, que es
-- `uuid`:
--
--   ERROR 42804: column "ref_id" is of type uuid but expression is of type text
--
-- No es un caso borde: PostgreSQL no admite la conversion text -> uuid en
-- contexto de asignacion, asi que el INSERT revienta al planificarse. Falla
-- SIEMPRE, se venda un producto o una linea escrita a mano (con ref_id nulo
-- tambien, porque el error es de tipos, no de valor). Verificado contra la BD.
--
-- Se colo al anadir el descuento de stock (pos-cobro-walkin-inventory): ahi si
-- se castea (`v_ref_id::uuid`) en el UPDATE de inventario y en el movimiento,
-- pero el INSERT de la linea se quedo sin castear.
--
-- El resto del cuerpo es identico al desplegado.
...
declare
  ...
  v_precio integer;
  v_cantidad integer;
  -- El id llega como texto en el JSON; se guarda ya convertido a uuid.
  v_ref_id uuid;
begin
  ...
  for v_linea in select * from jsonb_array_elements(p_lineas) loop
    -- Conversion explicita a uuid: aqui estaba el fallo.
    ...
    -- Si es producto, descuenta stock y deja el movimiento de inventario.
