-- Migracion: bloquear clientes — C5
-- Proyecto Supabase Mecha: vtrggiogjrhqtwbhbgia
--
-- Permite marcar un cliente como bloqueado (p. ej. no-shows reincidentes).
-- Un cliente bloqueado no puede reservar por el portal publico (se refuerza
-- crear_cita_publica). En la agenda interna el equipo ve el aviso (UI).

alter table public.clientes add column if not exists bloqueado boolean not null default false;
alter table public.clientes add column if not exists bloqueo_motivo text;

-- Refuerzo de crear_cita_publica: rechazar si el telefono corresponde a un
-- cliente bloqueado del negocio. (Resto identico a portal-reserva-publica.sql.)
...
begin
  ...
  if p_cliente_telefono is not null and length(trim(p_cliente_telefono)) > 0 and exists (
