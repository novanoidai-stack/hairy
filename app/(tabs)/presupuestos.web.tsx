...
const ANIM = `
  ...
  .p-row:hover { background: rgba(244,80,30,0.06) !important; }
  .p-btn { transition: transform 0.15s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.15s ease, filter 0.15s ease, box-shadow 0.15s ease; cursor: pointer; }
  .p-btn:hover:not(:disabled) { transform: translateY(-1px); filter: brightness(1.04); box-shadow: 0 4px 14px rgba(40,30,24,0.12); }
  .p-btn:active:not(:disabled) { transform: translateY(0) scale(0.97); opacity: 0.9; transition-duration: 0.08s; }
  ...
  return (
    <div className="p-modal-overlay" onClick={() => { if (!busy) onClose(); }}
      ...
      <div className="p-modal" onClick={e => e.stopPropagation()}
        style={{ background: T.panel, border: `1px solid ${T.borderHi}`, borderRadius: isMobile ? '16px 16px 0 0' : 16, padding: isMobile ? '18px 18px 84px' : 24, width: '100%', maxWidth: 560, maxHeight: isMobile ? '94vh' : 'calc(92vh / var(--mecha-zoom, 1))', overflowY: 'auto', boxShadow: '0 24px 70px rgba(40,30,24,0.35)' }}>
        ...
        <select className="m-control" value={profesionalId} onChange={e => setProfesionalId(e.target.value)} style={{ ...inputBase, marginBottom: 14 }}>
        ...
        {chispaBloques.length > 0 && (
          <div style={{ marginTop: 20, marginBottom: 10, background: '#FFF7E6', border: '1px solid #FFD591', borderRadius: 12, padding: 16 }}>
            ...
            {chispaBloques.map((b, i) => (
              <BloqueRenderer key={i} bloque={b} onConfirmar={async () => {}} isMobile={isMobile} />
            ))}
          </div>
        )}
...
function PresupuestosScreen() {
  ...
  return (
    <div style={{ background: T.bg, height: 'calc(100vh / var(--mecha-zoom, 1))', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <style>{ANIM}</style>
